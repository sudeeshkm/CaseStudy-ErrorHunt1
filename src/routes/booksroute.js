...
booksRouter.post('/delete', function (req, res) {
    ...
    bookdata.deleteOne({ _id: id }) //#Part 2 Point 1 
        .then(function () {
...
const id = req.body.id;    
    bookdata.findById({ _id: id }, function(err, data){
        if (err) {
            throw err;
        }
        else {
...
const id = req.body.id;  //#Part 2 Point 1 
    bookdata.updateOne({ _id: id }, { $set: req.body }, function (err, data) {
        if (err) {
